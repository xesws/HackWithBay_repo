...
sleep 1
setsid bash -c 'SCORER_REFERENCE_BACKEND=auto PYTHONPATH='"$REPO"' nohup .venv/bin/python -c "from dotenv import load_dotenv; load_dotenv(\"'"$REPO"'/.env\"); import uvicorn; uvicorn.run(\"scorer.app:app\", host=\"0.0.0.0\", port=8888)" >> '"$REPO"'/scorer.log 2>&1' < /dev/null &
